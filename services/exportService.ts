...
export const exportToPDF = (paper: QuestionPaper) => {
  ...
  if (shortAns.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: [
        [{ content: 'Short Answer Type Questions (Attempt any two)', colSpan: 3, styles: { fontStyle: 'bold', fillColor: [240, 240, 240], textColor: 0 } }],
        ...shortAns.map((q, i) => [
          {
            content: `${i + 1}. ${q.text}${q.imageUrl ? '\n\n\n\n\n\n\n\n\n\n' : ''}`,
            styles: { cellPadding: 3, textColor: 0 }
          },
          { content: '2', styles: { textColor: 0, valign: 'middle' as const, halign: 'center' as const } },
          { content: q.courseOutcome || '3', styles: { textColor: 0, valign: 'middle' as const, halign: 'center' as const } }
        ])
      ],
      margin: { left: margin, right: margin },
      theme: 'grid',
      ...
      didDrawCell: (data) => {
        if (data.section === 'body' && data.column.index === 0) {
          ...
          if (q && q.imageUrl) {
            ...
            const imgHeight = 30;
            const x = data.cell.x + (data.cell.width - imgWidth) / 2;
            const y = data.cell.y + (data.cell.height * 0.15);
            
            try {
              console.log("🖼️ Adding Short Answer image to PDF");
              doc.addImage(q.imageUrl, 'PNG', x, y, imgWidth, imgHeight);
              console.log("✅ Short Answer image added successfully");
            } catch (e) {
              console.error("❌ Error adding Short Answer image to PDF:", e);
              // Add a placeholder rectangle if image fails
              doc.rect(x, y, imgWidth, imgHeight);
              doc.setFontSize(8);
              doc.text('Image', x + imgWidth/2, y + imgHeight/2, { align: 'center' });
            }
          }
        }
      }
    ...
    autoTable(doc, {
      ...
      body: longAns.map((q, i) => {
        ...
        return [
          ...
          { 
            content: `a) ${q.text}${q.imageUrl ? '\n\n\n\n\n\n\n\n\n\n\n\n' : ''}\n\n\n\n\n\n\n\n\n\n\n\nb) ${alternativeText}`, 
            styles: { cellPadding: 5, textColor: 0, cellWidth: 'auto' } 
          },
          { content: q.bloomLevel || 'Analyze (4)', styles: { textColor: 0, valign: 'middle' as const, halign: 'center' as const } },
          { content: '7', styles: { textColor: 0, valign: 'middle' as const, halign: 'center' as const } },
          { content: q.courseOutcome || '3', styles: { textColor: 0, valign: 'middle' as const, halign: 'center' as const } }
        ];
      }),
      margin: { left: margin, right: margin },
      ...
      didDrawCell: (data) => {
        if (data.section === 'body' && data.column.index === 1) {
          ...
          if (q) {
            ...
            const orY = data.cell.y + (data.cell.height * 0.65); 
            doc.text('OR', data.cell.x + data.cell.width / 2, orY, { align: 'center' });
            doc.setFont('times', 'normal');
            doc.setFontSize(9);

            // Add image for part a (main question)
            if (q.imageUrl) {
              const imgWidth = 40;
              const imgHeight = 30;
              const x = data.cell.x + (data.cell.width - imgWidth) / 2;
              ...
              try {
                console.log("🖼️ Adding image to PDF for question:", q.text.substring(0, 30));
                doc.addImage(q.imageUrl, 'PNG', x, y, imgWidth, imgHeight);
            ...
            // Add image for part b (alternative question)
            if (q.alternativeImageUrl) {
              const altImgWidth = 40;
              const altImgHeight = 30;
              const altX = data.cell.x + (data.cell.width - altImgWidth) / 2;
              // Position image after 'OR' text with proper spacing
              const altY = orY + 10; 
              
              try {
                console.log("🖼️ Adding alternative image to PDF");
                doc.addImage(q.alternativeImageUrl, 'PNG', altX, altY, altImgWidth, altImgHeight);
                console.log("✅ Alternative image added successfully");
              } catch (e) {
                console.error("❌ Error adding alternative image to PDF:", e);
                // Add a placeholder rectangle if image fails
                doc.rect(altX, altY, altImgWidth, altImgHeight);
                doc.setFontSize(8);
                doc.text('Alt Image', altX + altImgWidth/2, altY + altImgHeight/2, { align: 'center' });
              }
            }
          }
        }
  ...
  autoTable(doc, {
    ...
    margin: { left: margin, right: margin },
    theme: 'grid',
    styles: { font: 'times', fontSize: 8, cellPadding: 2, lineColor: 0, lineWidth: 0.1 },
    ...
    sections: [
      {
        ...
        children: [
          ...
          ...paper.questions.filter(q => q.type === QuestionType.MCQ).flatMap((q, i) => [
            new Paragraph({
              children: [new TextRun({ text: `${i + 1}. ${q.text} [0.5 Marks] [CO: ${q.courseOutcome || '3'}]`, bold: true })],
              spacing: { before: 200 },
            }),
            ...(q.imageUrl ? [
              new Paragraph({
          ...
          ...paper.questions.filter(q => q.type === QuestionType.SHORT_ANSWER).flatMap((q, i) => [
            new Paragraph({
              children: [new TextRun({ text: `${i + 1}. ${q.text} [2 Marks] [CO: ${q.courseOutcome || '3'}]`, bold: true })],
              spacing: { before: 200 },
            }),
            ...(q.imageUrl ? [
              new Paragraph({
          ...
          ...paper.questions.filter(q => q.type === QuestionType.LONG_ANSWER).flatMap((q, i) => [
            new Paragraph({
              children: [new TextRun({ text: `${i + 1}. a) ${q.text} [7 Marks] [CO: ${q.courseOutcome || '3'}] [Bloom: ${q.bloomLevel || 'Analyze'}]`, bold: true })],
              spacing: { before: 200 },
            }),
            ...(q.imageUrl ? [
              new Paragraph({
            ...
            new Paragraph({
              children: [new TextRun({ text: `b) ${q.alternativeText || generateAlternativeQuestion(q.text, q.type, usedAlternatives)}`, bold: true })],
            }),
            ...(q.alternativeImageUrl ? [
              new Paragraph({
                children: [
                  new ImageRun({
                    data: getUint8ArrayFromBase64(getBase64Data(q.alternativeImageUrl)),
                    transformation: { width: 200, height: 150 },
                  } as any),
                ],
                alignment: AlignmentType.CENTER,
...
export const exportToTxt = (paper: QuestionPaper) => {
  ...
  paper.questions.filter(q => q.type === QuestionType.LONG_ANSWER).forEach((q, i) => {
    ...
    content += `${i + 1}. a) ${q.text} [7 Marks]\n`;
    if (q.imageUrl) content += `   [Visual Element: ${q.imageDescription || 'Image extracted'}]\n`;
    content += `   OR\n`;
    content += `   b) ${alternativeText}\n\n`;
  });
